...
declare global {
  type ICustomEventEmitter<MyEvents extends Record<string | symbol, (...args: any[]) => void>> =
    ...
    > & {
      ...
      once<K extends keyof MyEvents>(
        event: K,
        listener: MyEvents[K],
      ): ICustomEventEmitter<MyEvents>;
      off<K extends keyof MyEvents>(event: K, listener: MyEvents[K]): ICustomEventEmitter<MyEvents>;
      addListener<K extends keyof MyEvents>(
        event: K,
        listener: MyEvents[K],
      ): ICustomEventEmitter<MyEvents>;
      removeListener<K extends keyof MyEvents>(
        event: K,
        listener: MyEvents[K],
      ): ICustomEventEmitter<MyEvents>;
      removeAllListeners<K extends keyof MyEvents>(event?: K): ICustomEventEmitter<MyEvents>;
      prependListener<K extends keyof MyEvents>(
        event: K,
        listener: MyEvents[K],
      ): ICustomEventEmitter<MyEvents>;
      prependOnceListener<K extends keyof MyEvents>(
        event: K,
        listener: MyEvents[K],
      ): ICustomEventEmitter<MyEvents>;
