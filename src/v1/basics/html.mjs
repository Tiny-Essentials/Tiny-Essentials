 ...
 * @param {(res: Response) => ResponseInit} [returnResponse] - The callback function to return the ResponseInit to ReadableStream.
 * @returns {Response} A new Response object with the tracked stream.
 */
export function trackFetchProgress(
  response,
  onProgress,
  returnResponse = (res) => ({
    headers: res.headers,
  ...
  return new Response(stream, returnResponse(response));
}
 ...
 */
export async function fetchJson(url, options, onProgress) {
  ...
  if (onProgress !== undefined && typeof onProgress !== 'function') {
    return Promise.reject(new TypeError('The "onProgress" option must be a function.'));
  }
 ...
 */
export async function fetchBlob(url, allowedMimeTypes, options, onProgress) {
  if (typeof url !== 'string') {
    return Promise.reject(new TypeError('The "url" argument must be a string.'));
  }
 ...
 * @throws {Error} Throws if fetch fails, response is not ok, or MIME type is not allowed.
 */
export async function fetchText(url, allowedMimeTypes, options, onProgress) {
  if (typeof url !== 'string') {
    return Promise.reject(new TypeError('The "url" argument must be a string.'));
  }
  if (allowedMimeTypes !== undefined && !Array.isArray(allowedMimeTypes)) {
